...
<div class="space-y-4">
    ...
    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm">
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50 transition">
                        <td class="px-4 py-3 text-gray-800">{{ \Carbon\Carbon::parse($entry->entry_date)->format('d/m/Y') }}</td>
                        <td class="px-4 py-3 text-right font-semibold text-gray-800">{{ number_format($entry->items->sum('qty'), 0, ',', '.') }}</td>
                        <td class="px-4 py-3 text-gray-600 max-w-xs">
                            {{ $entry->items->map(fn($i) => ($i->product->{'nama_produk'} ?? '?').' ('.$i->qty.')')->join(', ') }}
                        </td>
                        ...
                        <td class="px-4 py-3 text-center">
                            <div class="flex items-center justify-center gap-2">
                                ...
                                <form method="POST" action="{{ route('stock-entries.destroy', $entry) }}" @submit.prevent>
                                    @csrf @method('DELETE')
                                    <button type="button"
                                            @click="openDeleteModal($el.closest('form'), 'Yakin ingin menghapus data stok masuk ini?')"
        ...
        <div class="px-4 py-3 border-t border-gray-100">{{ $stockEntries->links() }}</div>
        @endif
    </div>
</div>
@endsection
