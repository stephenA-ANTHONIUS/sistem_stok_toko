...
<div class="max-w-3xl" x-data="stockForm(
    ...
    {{ $stockEntry->items->map(fn($i) => [
        'product_id'  => $i->product_id,
        'nama_produk' => $i->product->nama_produk ?? 'Produk Tidak Diketahui',
        'jumlah_stok' => $i->product->jumlah_stok ?? 0,
        'qty'         => $i->qty
    ])->values() }}
)">
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6 space-y-6">
        <h2 class="text-base font-bold text-gray-800">Form Edit Stok Masuk</h2>
        ...
        <form method="POST" action="{{ route('stock-entries.update', $stockEntry) }}" class="space-y-5">
            ...
            <div class="grid sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="date" name="entry_date"
                           value="{{ old('entry_date', \Carbon\Carbon::parse($stockEntry->entry_date)->format('Y-m-d')) }}" required
                ...
                <div>
                    ...
                    <input type="text" name="notes" value="{{ old('notes', $stockEntry->notes) }}"
                           class="w-full px-4 py-2.5 border border-gray-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
            </div>
            ...
            <div>
                ...
                <p class="text-xs text-gray-500 mb-3">Cari dan klik tombol tambah untuk menambahkan produk ke daftar.</p>
                    ...
                    <template x-for="(item, index) in items" :key="index">
                        <div class="flex items-center gap-3 bg-gray-50 rounded-xl px-4 py-3">
                            ...
                            <div class="flex-1">
                                ...
                                <p class="text-xs text-gray-400">Stok saat ini: <span x-text="item.jumlah_stok"></span></p>
                            </div>
...
function stockForm(initialProducts, initialItems = []) {
    return {
        ...
        addProduct(product) {
            const existing = this.items.find(i => i.product_id === product.id || String(i.product_id) === String(product.id));
            if (existing) {
