...
@section('content')
<div class="max-w-3xl" x-data="editStockForm({{ json_encode($stockEntry->items->map(fn($i) => ['product_id' => (string)$i->product_id, 'qty' => $i->qty])->values()) }})">
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6 space-y-6">
        ...
        <form method="POST" action="{{ route('stock-entries.update', $stockEntry) }}" class="space-y-5">
            ...
            <div class="flex gap-3 pt-2">
                ...
                <a href="{{ route('stock-entries.index') }}" class="px-6 py-2.5 bg-gray-100 hover:bg-gray-200 text-gray-700 text-sm font-semibold rounded-xl transition">Batal</a>
            </div>
        </form>
    </div>
</div>
<script>
function editStockForm(initialItems) {
    return {
