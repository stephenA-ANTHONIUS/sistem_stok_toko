...
@section('page-title', 'Edit Rekap Resi')

@section('content')
<div class="max-w-3xl" x-data="editItems({{ json_encode($shippingLabel->items ?? []) }}, {{ json_encode($products->toArray()) }})">
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6 space-y-6">
        <h2 class="text-base font-bold text-gray-800">Edit Rekap Produk Resi</h2>

        <form method="POST" action="{{ route('shipping-labels.update', $shippingLabel) }}" class="space-y-5">
            @csrf @method('PUT')
            ...
            <div class="bg-gray-50 rounded-xl p-4">
                <details>
                    ...
                    <pre class="mt-3 text-xs text-gray-500 whitespace-pre-wrap max-h-60 overflow-y-auto">{{ $shippingLabel->raw_text }}</pre>
                </details>
            </div>
            ...
            <div class="flex gap-3">
                ...
                <a href="{{ route('shipping-labels.index') }}" class="px-6 py-2.5 bg-gray-100 hover:bg-gray-200 text-gray-700 text-sm font-semibold rounded-xl transition">
                    Batal
                </a>
            </div>
        </form>
