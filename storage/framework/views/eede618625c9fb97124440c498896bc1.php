...
<?php $__env->startSection('title', 'Preview Resi'); ?>
<?php $__env->startSection('page-title', 'Preview Hasil Scan Resi'); ?>
...
<?php if(empty($mappedItems) && empty($data['raw_text'])): ?>
    <div class="max-w-2xl">
...
<div class="max-w-4xl" x-data="previewItems(
    <?php echo e(json_encode(array_map(fn($i) => ['produk' => $i['produk'] ?? '', 'qty' => $i['qty'] ?? 1, 'harga_online' => $i['harga_online'] ?? 0, 'stock' => $i['stock'] ?? 0, 'original' => $i['original'] ?? $i['produk'], 'matched' => $i['matched'] ?? false], $mappedItems))); ?>,
    <?php echo e(json_encode($products->map(fn($p) => ['id' => $p->id, 'nama_produk' => $p->nama_produk, 'harga_online' => $p->harga_online ?? $p->harga ?? 0, 'stock' => $p->jumlah_stok ?? 0])->values())); ?>
    ...
    <?php $unmatchedCount = count(array_filter($mappedItems, fn($i) => !($i['matched'] ?? false))); ?>
    ...
    <div class="mb-4 flex items-start gap-3 bg-amber-50 border border-amber-200 text-amber-800 text-sm px-4 py-3 rounded-xl">
        <i class="fa-solid fa-triangle-exclamation mt-0.5 shrink-0"></i>
        <p>Terdapat <strong><?php echo e($unmatchedCount); ?> produk</strong> yang tidak ditemukan di database. Silakan pilih produk yang sesuai dari dropdown atau hapus baris tersebut.</p>
    </div>
    <?php endif; ?>
    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6 space-y-6">
        ...
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-xs text-gray-500 mt-0.5">
                    File: <span class="font-medium"><?php echo e(basename($data['image_path'])); ?></span>
        ...
        <form method="POST" action="<?php echo e(route('shipping-labels.confirm')); ?>" class="space-y-5">
            <?php echo csrf_field(); ?>

            <input type="hidden" name="image_path" value="<?php echo e($data['image_path']); ?>">
            <input type="hidden" name="raw_text" value="<?php echo e($data['raw_text']); ?>">
            ...
            <div class="bg-gray-50 rounded-xl p-4 mt-4">
                <details>
                    ...
                    <pre class="mt-3 text-xs text-gray-500 whitespace-pre-wrap max-h-60 overflow-y-auto font-mono leading-relaxed"><?php echo e($data['raw_text']); ?></pre>
                </details>
            </div>
            ...
            <div class="flex gap-3 pt-2 mt-4">
                ...
                <a href="<?php echo e(route('shipping-labels.create')); ?>" class="px-6 py-2.5 bg-gray-100 hover:bg-gray-200 text-gray-700 text-sm font-semibold rounded-xl transition flex items-center gap-2">
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\sistem_stok_toko\resources\views/shipping-labels/preview.blade.php ENDPATH**/ ?>